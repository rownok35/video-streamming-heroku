...
def user(request, username):
    ...
    already_followed = Follow.objects.filter(
        subscriber=request.user, subscribing=user_other)
...
def follow(request, username):
    subscribing_user = User.objects.get(username=username)
    subscriber_user = request.user
    already_followed = Follow.objects.filter(
        subscriber=subscriber_user, subscribing=subscribing_user)
    if not already_followed:
        followed_user = Follow(subscriber=subscriber_user,
